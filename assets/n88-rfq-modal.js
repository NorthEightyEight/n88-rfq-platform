...
(function() {
    ...
    const N88PDFExtraction = {
        ...
        init: function() {
            ...
            // Entry mode toggle - use event delegation for better reliability
            const form = document.getElementById('n88-rfq-form') || document.getElementById('n88-sourcing-form');
            if (form) {
                form.addEventListener('change', (e) => {
                    if (e.target.classList.contains('entry-mode-radio')) {
                        this.handleModeChange(e);
                    }
                });
            }
            
            // Also attach directly to radios as backup
            ...
            if (pdfDropzone) {
                pdfDropzone.addEventListener('click', () => {
                    document.getElementById('pdf_file_upload').click();
                });

                pdfDropzone.addEventListener('dragover', (e) => {
                    e.preventDefault();
                    pdfDropzone.classList.add('n88-drag-over');
                });

                pdfDropzone.addEventListener('dragleave', () => {
                    pdfDropzone.classList.remove('n88-drag-over');
                });
            ...
            if (confirmBtn) {
                confirmBtn.addEventListener('click', this.confirmExtraction.bind(this));
            }

            // Cancel extraction button
            const cancelBtn = document.getElementById('cancel-extraction-btn');
            if (cancelBtn) {
                cancelBtn.addEventListener('click', this.cancelExtraction.bind(this));
            }
        },
        ...
        handleModeChange: function(e) {
            ...
            if (mode === 'manual') {
                ...
                if (pdfMode) {
                    ...
                    console.log('PDF mode shown');
                    
                    // Check if project_id exists, if not, show warning
                    const projectId = this.getProjectId();
                    if (!projectId) {
                        const warningDiv = document.createElement('div');
                        warningDiv.className = 'n88-pdf-warning';
                        warningDiv.style.cssText = 'margin-top: 15px; padding: 12px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px; color: #856404;';
                        warningDiv.innerHTML = '<strong>Note:</strong> Please save the project as a draft first before uploading PDF for extraction.';
                        ...
                        // Insert warning after the info paragraph
                        const infoPara = pdfMode.querySelector('.n88-pdf-upload-info');
                        if (infoPara && infoPara.nextSibling) {
                            pdfMode.insertBefore(warningDiv, infoPara.nextSibling);
                        } else {
                            pdfMode.appendChild(warningDiv);
                        }
                    }
                }
            }
        ...
        handlePDFUpload: function(file) {
            const projectId = this.getProjectId();
            if (!projectId) {
                const createDraft = confirm('You need to save this project as a draft first before uploading PDF. Would you like to save it now?');
                if (createDraft) {
                    // Trigger form submission to save as draft
                    const form = document.getElementById('n88-rfq-form') || document.getElementById('n88-sourcing-form');
                    if (form) {
                        const submitBtn = form.querySelector('button[name="submit_type"][value="draft"]');
                        if (submitBtn) {
                            // Store the file temporarily and submit form
                ...
                alert('Please save the project as a draft first, then you can upload the PDF.');
                return;
            }
        ...
        showExtractionPreview: function(extractionData) {
            if (typeof N88PDFExtraction !== 'undefined' && typeof N88PDFExtraction.showExtractionPreview === 'function') {
                N88PDFExtraction.showExtractionPreview(extractionData);
            } else {
                console.error('N88PDFExtraction.showExtractionPreview not available, ensure assets/n88-rfq-pdf-extraction.js is loaded.');
            }
        },
        ...
        confirmExtraction: function() {
            ...
            const formData = new URLSearchParams({
                action: 'n88_confirm_extraction',
                items: JSON.stringify(this.currentExtractionData.items),
                nonce: n88.nonce
            });
            ...
            .then(data => {
                if (data.success) {
                    alert('Items extracted successfully! The page will reload.');
                    window.location.reload();
                } else {
                    alert('Error: ' + (data.data?.message || 'Failed to confirm extraction'));
                }
            })
            .catch(error => {
                console.error('Confirm extraction error:', error);
                alert('Error confirming extraction. Please try again.');
            });
        },
    ...
    // Initialize PDF extraction when DOM is ready
    function initPDFExtraction() {
        if (typeof N88PDFExtraction !== 'undefined') {
            N88PDFExtraction.init();
            N88PDFExtraction.initialized = true;
        }
    }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(initPDFExtraction, 100);
        });
    } else {
        setTimeout(initPDFExtraction, 100);
    }
    ...
    window.addEventListener('load', () => {
        if (!N88PDFExtraction.initialized) {
            setTimeout(initPDFExtraction, 200);
        }
    });
